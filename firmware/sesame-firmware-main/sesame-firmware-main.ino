...
// ======================================================================
// Connect the eight servos to PCA9685 channels 0-7 in the same order used
// by movement-sequences.h. The PCA9685 and OLED share the I2C bus.
// ======================================================================
...
void setup() {
  ...
  // Use the board's default I2C pins, matching the working spider sketch.
  // A classic ESP32 uses SDA 21 and SCL 22 by default.
  Wire.begin();

  // OLED Init
  uint8_t oledAddress = detectOledAddress();
  ...
  // PCA9685 servo driver init. It shares SDA/SCL with the OLED at 0x3C/0x3D.
  if (!pwm.begin()) {
    Serial.println(F("PCA9685 not found at 0x40."));
    while (1);
  }
  pwm.setPWMFreq(SERVO_FREQUENCY_HZ);
  Serial.println(F("PCA9685 initialized at 0x40, servo frequency 50 Hz"));
  delay(10);
...
uint8_t detectOledAddress() {
  if (i2cDevicePresent(OLED_I2C_ADDR)) return OLED_I2C_ADDR;
  if (i2cDevicePresent(OLED_I2C_ADDR_ALT)) return OLED_I2C_ADDR_ALT;
...
void setServoAngle(uint8_t channel, int angle) {
  if (channel < 8) {
    ...
    uint16_t pulseWidthUs = map(adjustedAngle, 0, 180, SERVO_MIN_US, SERVO_MAX_US);
    pwm.writeMicroseconds(channel, pulseWidthUs);
